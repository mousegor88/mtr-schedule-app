...
export default function QubScreen() {
  const router = useRouter();
  const [upRaw, setUpRaw] = useState<RawTrain[]>([]);
  ...
  const renderSection = (title: string, trains: DisplayTrain[]) => (
    <View style={styles.section}>
      ...
      {trains.map((train) => (
        <View key={`${title}-${train.seq}`} style={styles.row}>
  ...
  return (
    <View style={styles.container}>
      ...
      <StationTopBar title="Quarry Bay" onBack={() => router.back()} />
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {!loading && !error ? (
          <>
            {renderSection('To LOHAS Park/Po Lam', upTrains)}
            {renderSection('To North Point', downTrains)}
          </>
        ) : null}
      </ScrollView>
